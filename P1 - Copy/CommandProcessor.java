...
// Title:            Program One
// Files:            Message.java, User.java, Helper.java, CommandProcessor.java
//						Message.java
...
public class CommandProcessor
{
    ...
    {
    	User friendTemp = null; //will later hold the friend the message is sent to
    	BroadcastList bcastTemp = null; //will hold broadcastlist to be sent to
        String finalUserName = null, finalBcastName = null; //Used in constructor
    	boolean isFriendName = false, isBcastName = false; 
    		//determines what the message is being sent to
    	Iterator<User> friendItr = CONFIG.getCurrentUser()
    			.getFriends().iterator();
    		//iterates through current user's friend list
    	Iterator<BroadcastList> bcastItr= CONFIG.getCurrentUser()
    			.getBroadcastLists().iterator();
    		//iterates through current user's broadcast lists
    	while (friendItr.hasNext()) //checks to see if it's a friend
    	{
    		friendTemp = friendItr.next();
    		if (friendTemp.getNickname().equals(nickname))
    		{
    			 isFriendName = true;
    			 finalUserName = friendTemp.getNickname();
    			 finalBcastName = null;
    			 break;
    		}
    	}
    	while (bcastItr.hasNext()) //checks to see if its a broadcast list
    	{
    		bcastTemp = bcastItr.next();
    		if (bcastTemp.getNickname().equals(nickname))
    		{
    			isBcastName = true;
    			finalBcastName = bcastTemp.getNickname();
    			finalUserName = null;
    			break;
    		}
    	}
     	if (nickname.equals(CONFIG.getCurrentUser().getNickname()))
    	{
    		throw new WhatsAppException(Config.CANT_SEND_YOURSELF);
    	}
    	if (!(isBcastName || isFriendName))
    	{
    		throw new WhatsAppException
    			(String.format(Config.NICKNAME_DOES_NOT_EXIST, nickname));
    	}
    	Date tempDate = new Date();
    	Message m = new Message(CONFIG.getCurrentUser().getNickname(),
    			finalUserName, finalBcastName,
                tempDate, message, false); 
    		//creates and holds the message object
    	
    	if(isFriendName) //adds a message to current and friend's messages
    	{
    		m.setToNickname(friendTemp.getNickname());
    		friendTemp.getMessages().add(m);
    		CONFIG.getCurrentUser().getMessages().add(m);
    	}
    	if(isBcastName)
    		//adds a message to everyone in broadcast list and current user
    	{
    		Iterator<String> nameItr = bcastTemp.getMembers().iterator();
    		while (nameItr.hasNext())
    		{
    			String nameTemp = nameItr.next();
    			User userTemp = Helper.getUserFromNickname
    					(CONFIG.getAllUsers(), nameTemp);
    			m.setToNickname(bcastTemp.getNickname());
    			userTemp.getMessages().add(m);
    		}
    		m.setBroadcastNickname(bcastTemp.getNickname());
    		CONFIG.getCurrentUser().getMessages().add(m);
    	}
    	Config.getInstance().getConsoleOutput().printf(
    			Config.MESSAGE_SENT_SUCCESSFULLY, m);
    ...
    public static void readMessage(String nickname, boolean enforceUnread)
    {
    	boolean hasMessages = false; 
    		//Used to check and later give input if there are no msgs later
    	if(nickname == null && !enforceUnread)
    	{
    		Iterator<Message> msgItr = CONFIG.getCurrentUser()
    				.getMessages().iterator();
    		//iterates through messages of this user
    		while(msgItr.hasNext())
    		{
    			Message tempMsg = msgItr.next();
    			if(tempMsg.getBroadcastNickname() == null)
    			{
    				CONFIG.getConsoleOutput().
    				printf(Config.MESSAGE_FORMAT, tempMsg.getFromNickname(),
						tempMsg.getToNickname(), tempMsg.getMessage(),
						tempMsg.getSentTime());
    				tempMsg.setRead(true);
    				hasMessages = true;
    			}
    			if(tempMsg.getToNickname() == null)
    			{
    				CONFIG.getConsoleOutput().
    				printf(Config.MESSAGE_FORMAT, tempMsg.getFromNickname(),
						tempMsg.getBroadcastNickname(), tempMsg.getMessage(),
						tempMsg.getSentTime());
    				tempMsg.setRead(true);
    				hasMessages = true;
    			}
    	...
    	{
    		Iterator<Message> msgItr = CONFIG.getCurrentUser().getMessages().iterator();
    		while(msgItr.hasNext())
    		{
    			Message tempMsg = msgItr.next();
    			if (!tempMsg.isRead())
    	...
    	else if(nickname != null && !enforceUnread)
    	{
    		...
    		{
    			Message tempMsg = msgItr.next();
    			if (tempMsg.getFromNickname().equals(nickname))
    				//^if there's an issue with reading nicknames
    			{
    				CONFIG.getConsoleOutput().
    				printf(Config.MESSAGE_FORMAT, tempMsg.getFromNickname(),
    						tempMsg.getToNickname(), tempMsg.getMessage(),
    	...
    	else if(nickname != null && enforceUnread)
    	{
    		...
    		{
    			Message tempMsg = msgItr.next();
    			if (!tempMsg.isRead() && tempMsg.getFromNickname().equals(nickname))
    			{
    				CONFIG.getConsoleOutput().
    				printf(Config.MESSAGE_FORMAT, tempMsg.getFromNickname(),
    						tempMsg.getToNickname(), tempMsg.getMessage(),
    ...
    {
    	word.toLowerCase();
    	boolean isFound = false; //used to signal what output to print
    	Iterator<User> userItr = CONFIG.getAllUsers().iterator();
    	...
    	if (searchByFirstName)
    	{
    		while(userItr.hasNext())
    		{
    			...
    			if (temp.getFirstName().contains(word))
    			{
    				String sTempIsFriend;
    				boolean tempIsFriend = 
    						CONFIG.getCurrentUser().isFriend(temp.getNickname());
    				if(tempIsFriend)
    					sTempIsFriend = "yes";
    				else
    					sTempIsFriend = "no";
    				
    				CONFIG.getConsoleOutput().printf(Config.USER_DISPLAY_FOR_SEARCH, 
    						temp.getLastName(), temp.getFirstName(), 
    						temp.getNickname(), sTempIsFriend);
    				isFound = true;
		...
		{
			User temp = userItr.next();
			if (temp.getLastName().contains(word))
			{
				String sTempIsFriend;
				boolean tempIsFriend = 
						CONFIG.getCurrentUser().isFriend(temp.getNickname());
    ...
    public static void addFriend(String nickname) throws WhatsAppException
    {
    	...
    	Config.getInstance().getConsoleOutput()
    		.printf(Config.SUCCESSFULLY_ADDED, nickname);
    }
    ...
    public static void removeFriendFromBcast(String friendNickname,
            String bcastNickname) throws WhatsAppException
    {
    	boolean bcastExists = false; 
    		//used to signal whether there is a bcastlist
    	boolean isFriend = false;
    		//used to signal if the friend exists in friend list
    	boolean friendInBcast = false;
    		//used to signal whether the friend is in the bcastlist
    	BroadcastList bcastTemp = null;
    		//holds the bcast list object
    	User friendTemp;
    		//holds the friend object
    	Iterator<BroadcastList> bcastItr = Config.getInstance().getCurrentUser()
    			.getBroadcastLists().iterator();
    		//iterates through current user's bcastlists
    	Iterator<User> isFriendItr = Config.getInstance().getCurrentUser()
    			.getFriends().iterator();
    		//iterates through user's existing friends
    	while(isFriendItr.hasNext())
    	{
    		friendTemp = isFriendItr.next();
    		{
    			if(friendTemp.getNickname().equals(friendNickname))
    			{
    				isFriend = true;
    				break;
    			}
    	...
    	{
    		bcastTemp = bcastItr.next();
    		if(bcastTemp.getNickname().equals(bcastNickname))
    		{
    			bcastExists = true;
    			break;
    		}
    	}
    	if (bcastExists = false)
    	{
    		throw new WhatsAppException(Config.BCAST_LIST_DOES_NOT_EXIST);
    	}
    	Iterator<String> friendItr =  bcastTemp.getMembers().iterator();
    	while (friendItr.hasNext())
    	{
    		String nicknameTemp = friendItr.next();
    		if(nicknameTemp.equals(friendNickname))
    		{
    			bcastTemp.getMembers().remove(friendNickname);
    			Config.getInstance()
    				.getConsoleOutput().printf(Config.SUCCESSFULLY_REMOVED);
    			
    			friendInBcast = true;
    		}
    	}
    	if (friendInBcast = false)
    	{
    ...
    {
    	boolean exists = false; //used to signal whether the bcastlist exists
    	Iterator<BroadcastList> bcastItr = 
    			Config.getInstance().getCurrentUser()
    			.getBroadcastLists().iterator();
    		//iterates through user's bcastlists
    	while (bcastItr.hasNext())
    	{
    		BroadcastList temp = bcastItr.next();
    	...
    	{
    		Config.getInstance().getConsoleOutput()
    			.printf(Config.SUCCESSFULLY_REMOVED);
    	}
    }
